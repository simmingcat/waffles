...
void setup() {
  ...
  song1 = minim.loadFile("Showtek ft. We Are Loud & Sonny Wilson - Booyah (Official Music Video).mp3");
...
void draw() {
  ...
  for(int i = 0; i < song1.bufferSize() - 1; i++)
  {
    ...
    line( x1, 100 + song1.left.get(i)*10, x2, 100 + song1.left.get(i+1)*10 );
   
  }
  
  //Draw a line to show where in the song playback is currently located
  float posx = map(song1.position(), 50, song1.length(), 10, width);
  stroke(0,200,0);
  ellipse(posx, 50, posx, 10);
